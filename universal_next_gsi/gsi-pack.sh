#!/bin/bash
# =============================================================================
# gsi-pack.sh — Package sparse ext4 image for Universal Next
# =============================================================================
...
if [ "$(id -u)" -ne 0 ]; then
    echo "Requires root to format final image!"
    exit 1
fi

TEMP_IMAGE=$(mktemp)

echo "=== Generating Ext4 Universal Next Storage (${IMAGE_SIZE}) ==="
...
echo "Creating Universal Native Segregation Layout..."
...
echo "Adding Compiled rootfs..."
cp -ap "${ROOTFS_DIR}"/* "${TARGET_DIR}/ubuntu/rootfs/"

umount "${TARGET_DIR}"
rm -rf "${TARGET_DIR}"

echo "Transforming RAW payload to sparse framework (img2simg)..."
if command -v img2simg > /dev/null 2>&1; then
    img2simg "${TEMP_IMAGE}" "${OUTPUT_IMAGE}"
else
    echo "Warning: img2simg missing. Returning raw ext4 image block!"
